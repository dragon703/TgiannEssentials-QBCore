...
RegisterServerEvent('qb-weed:server:removeSeed', function(itemslot, seed)
    ...
    exports['tgiann-inventory']:RemoveItem(src, seed, 1, itemslot, 'qb-weed:server:removeSeed')
end)
    ...
    if weedBag ~= nil and weedBag.amount >= sndAmount then
        if house ~= nil then
            ...
            if result[1] ~= nil then
                exports['tgiann-inventory']:AddItem(src, 'weed_' .. plantName .. '_seed', amount, false, false, 'qb-weed:server:harvestPlant')
                exports['tgiann-inventory']:AddItem(src, 'weed_' .. plantName, sndAmount, false, false, 'qb-weed:server:harvestPlant')
                exports['tgiann-inventory']:RemoveItem(src, 'empty_weed_bag', sndAmount, false, 'qb-weed:server:harvestPlant')
...
RegisterNetEvent('qb-weed:server:foodPlant', function(house, amount, plantName, plantId)
    ...
    exports['tgiann-inventory']:RemoveItem(src, 'weed_nutrition', 1, false, 'qb-weed:server:foodPlant')
    TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, house)
end)
