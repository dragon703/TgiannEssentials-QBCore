...
RegisterNetEvent('equip:harness', function(item)
    ...
    elseif Player.PlayerData.items[item.slot].info.uses == 1 then
        exports['tgiann-inventory']:RemoveItem(src, 'harness', 1, false, 'equip:harness')
...
RegisterNetEvent('seatbelt:DoHarnessDamage', function(hp, data)
    ...
    if not Player then return end
    if hp == 0 then
        exports['tgiann-inventory']:RemoveItem(src, 'harness', 1, data.slot, 'seatbelt:DoHarnessDamage')
    else
