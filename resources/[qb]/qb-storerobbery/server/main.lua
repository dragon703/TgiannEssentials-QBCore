...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        ...
        exports['tgiann-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            exports['tgiann-inventory']:AddItem(src, 'stickynote', 1, false, info, 'qb-storerobbery:server:takeMoney')
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    exports['tgiann-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:SafeReward')
    TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add')
    local luck = math.random(1, 100)
    local odd = math.random(1, 100)
    if luck <= 10 then
        exports['tgiann-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'qb-storerobbery:server:SafeReward')
        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['tgiann-inventory']:AddItem(src, 'goldbar', 1, false, false, 'qb-storerobbery:server:SafeReward')
...
RegisterNetEvent('qb-storerobbery:server:removeAdvancedLockpick', function()
    ...
    if not Player then return end
    exports['tgiann-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'qb-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('qb-storerobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['tgiann-inventory']:RemoveItem(source, 'lockpick', 1, false, 'qb-storerobbery:server:removeLockpick')
end)
