...
RegisterServerEvent('qb-crafting:server:removeMaterials', function(itemName, amount)
    ...
    if Player then
        exports['tgiann-inventory']:RemoveItem(src, itemName, amount, false, 'qb-crafting:server:removeMaterials')
...
RegisterNetEvent('qb-crafting:server:removeCraftingTable', function(benchType)
    ...
    exports['tgiann-inventory']:RemoveItem(src, benchType, 1, false, 'qb-crafting:server:removeCraftingTable')
...
RegisterNetEvent('qb-crafting:server:addCraftingTable', function(benchType)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if not exports['tgiann-inventory']:AddItem(src, benchType, 1, false, false, 'qb-crafting:server:addCraftingTable') then return end
...
RegisterNetEvent('qb-crafting:server:receiveItem', function(craftedItem, requiredItems, amountToCraft, xpGain, xpType)
    ...
    for _, requiredItem in ipairs(requiredItems) do
        if not exports['tgiann-inventory']:RemoveItem(src, requiredItem.item, requiredItem.amount, false, 'qb-crafting:server:receiveItem') then
            canGive = false
            return
        end
    ...
    if canGive then
        if not exports['tgiann-inventory']:AddItem(src, craftedItem, amountToCraft, false, false, 'qb-crafting:server:receiveItem') then return end
