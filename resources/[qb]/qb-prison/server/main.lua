...
RegisterNetEvent('prison:server:GiveJailItems', function(escaped)
    ...
    for _, v in pairs(Player.PlayerData.metadata['jailitems']) do
        exports['tgiann-inventory']:AddItem(src, v.name, v.amount, false, v.info, 'prison:server:GiveJailItems')
    end
    Player.Functions.SetMetaData('jailitems', {})
end)
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    if not exports['tgiann-inventory']:AddItem(src, 'phone', 1, false, false, 'prison:server:CheckChance') then return end
