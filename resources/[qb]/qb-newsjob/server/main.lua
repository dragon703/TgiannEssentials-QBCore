...
RegisterNetEvent('qb-newsjob:server:addVehicleItems', function(plate)
    ...
    exports['tgiann-inventory']:CreateInventory('trunk-' .. plate)

    for slot, item in pairs(Config.VehicleItems) do
        exports['tgiann-inventory']:AddItem('trunk-' .. plate, item.name, item.amount, slot, item.info, 'qb-newsjob:vehicleItems')
    end
end)
