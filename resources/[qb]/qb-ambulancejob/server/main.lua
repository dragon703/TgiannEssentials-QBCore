...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' then
			exports['tgiann-inventory']:RemoveItem(src, 'bandage', 1, false, 'hospital:server:TreatWounds')
		...
		if Player.PlayerData.job.name == 'ambulance' or QBCore.Functions.HasItem(src, 'firstaid', 1) then
			if oldMan then
				if Player.Functions.RemoveMoney('cash', 5000, 'revived-player') then
					exports['tgiann-inventory']:RemoveItem(src, 'firstaid', 1, false, 'hospital:server:RevivePlayer')
...
RegisterNetEvent('hospital:server:removeBandage', function()
	...
	exports['tgiann-inventory']:RemoveItem(source, 'bandage', 1, false, 'hospital:server:removeBandage')
end)

RegisterNetEvent('hospital:server:removeIfaks', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['tgiann-inventory']:RemoveItem(source, 'ifaks', 1, false, 'hospital:server:removeIfaks')
end)

RegisterNetEvent('hospital:server:removePainkillers', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['tgiann-inventory']:RemoveItem(source, 'painkillers', 1, false, 'hospital:server:removePainkillers')
end)
