...
RegisterNetEvent('qb-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    ...
    if Config.Dealers[dealer]['products'][itemData.slot].amount - 1 >= 0 then
        ...
        exports['tgiann-inventory']:RemoveItem(src, itemData.name, amount, false, 'qb-drugs:server:updateDealerItems')
...
RegisterNetEvent('qb-drugs:server:giveDeliveryItems', function(deliveryData)
    ...
    exports['tgiann-inventory']:AddItem(src, item, deliveryData.amount, false, false, 'qb-drugs:server:giveDeliveryItems')
...
RegisterNetEvent('qb-drugs:server:successDelivery', function(deliveryData, inTime)
    ...
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            exports['tgiann-inventory']:RemoveItem(src, item, itemAmount, false, 'qb-drugs:server:successDelivery')
            if copsOnline > 0 then
                ...
                if Config.UseMarkedBills then
                    ...
                    exports['tgiann-inventory']:AddItem(src, 'markedbills', 1, false, info, 'qb-drugs:server:successDelivery')
                else
            ...
            if invItem then
                ...
                exports['tgiann-inventory']:RemoveItem(src, item, newItemAmount, false, 'qb-drugs:server:successDelivery')
                TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items[item], 'remove')
...
RegisterNetEvent('qb-drugs:server:dealerShop', function(currentDealer)
    ...
    exports['tgiann-inventory']:OpenShop(src, dealerData.name)
end)
