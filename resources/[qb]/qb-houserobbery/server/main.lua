    ...
    if itemCount > 0 then
        for _ = 1, itemCount do
            ...
            if not itemInfo.unique then
                ...
                exports['tgiann-inventory']:AddItem(src, selectedItem.item, amount, false, false, 'qb-houserobbery:server:searchFurniture')
            else
                exports['tgiann-inventory']:AddItem(src, selectedItem.item, 1, false, false, 'qb-houserobbery:server:searchFurniture')
            end
...
RegisterNetEvent('qb-houserobbery:server:removeAdvancedLockpick', function()
    ...
    exports['tgiann-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'qb-houserobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('qb-houserobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['tgiann-inventory']:RemoveItem(source, 'lockpick', 1, false, 'qb-houserobbery:server:removeLockpick')
end)
