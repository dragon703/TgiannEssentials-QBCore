...
RegisterNetEvent('qb-vineyard:server:getGrapes', function()
    ...
    exports['tgiann-inventory']:AddItem(source, 'grape', amount, false, false, 'qb-vineyard:server:getGrapes')
    ...
    if Player.PlayerData.items ~= nil then
        if grape ~= nil then
            if grape.amount >= 23 then
                exports['tgiann-inventory']:RemoveItem(src, 'grapejuice', 23, false, 'qb-vineyard:server:loadIngredients')
    ...
    if Player.PlayerData.items ~= nil then
        if grape ~= nil then
            if grape.amount >= 16 then
                exports['tgiann-inventory']:RemoveItem(src, 'grape', 16, false, 'qb-vineyard:server:grapeJuice')
...
RegisterNetEvent('qb-vineyard:server:receiveWine', function()
    ...
    exports['tgiann-inventory']:AddItem(source, 'wine', amount, false, false, 'qb-vineyard:server:receiveWine')
...
RegisterNetEvent('qb-vineyard:server:receiveGrapeJuice', function()
    ...
    exports['tgiann-inventory']:AddItem(source, 'grapejuice', amount, false, false, 'qb-vineyard:server:receiveGrapeJuice')
