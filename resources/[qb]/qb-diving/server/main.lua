...
RegisterNetEvent('qb-diving:server:SellCorals', function()
    ...
    if hasCoral(src) then
        for _, v in pairs(AvailableCorals) do
            ...
            exports['tgiann-inventory']:RemoveItem(src, item.name, item.amount, false, 'qb-diving:server:SellCorals')
...
RegisterNetEvent('qb-diving:server:TakeCoral', function(area, coral, bool)
    ...
    if amount > 1 then
        for _ = 1, amount, 1 do
            exports['tgiann-inventory']:AddItem(src, ItemData['name'], 1, false, false, 'qb-diving:server:TakeCoral')
            TriggerClientEvent('qb-inventory:client:ItemBox', src, ItemData, 'add')
            Wait(250)
        end
    else
        exports['tgiann-inventory']:AddItem(src, ItemData['name'], amount, false, false, 'qb-diving:server:TakeCoral')
        TriggerClientEvent('qb-inventory:client:ItemBox', src, ItemData, 'add')
    end
...
RegisterNetEvent('qb-diving:server:removeItemAfterFill', function()
    local src = source
    exports['tgiann-inventory']:RemoveItem(src, 'diving_fill', 1, false, 'qb-diving:server:removeItemAfterFill')
